...
plot_a_metabolite <- function(which){
...
```{r plot 1:27}
plot_metabolites(1:25)
plot_metabolites(26:55)
```
...
asca.fit <- ASCA.2f(X = t(df.final),Designa = mx$j, Designb = mx$i,type = 1, Fac = c(1,2,2,2))
Fac=c(1,2,2,2)
type=1
```

```{r,results=T}
asca.selected <- plot.leverage_spe(df.final,asca.fit,colnames,2)
print(asca.selected)
```
```{r}
