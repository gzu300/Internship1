...
```
```{r,include=F}
AA <- df.final.masigpro["CMP",]
PlotGroups(AA,edesign = edesign, show.fit = T, dis = dis, groups.vector = design$groups.vector,main = 'CMP')
```
...
PlotGroups(AA,edesign = edesign, show.fit = T, dis = dis, groups.vector = design$groups.vector,main = 'ATP')
```
...
```{r,results=T}
asca.fitted <- fitted(df.final,asca.fit,groups = colnames,100,ASCA.2f_leverage)
asca.selected <- plot.leverage_spe(asca.fitted,2,size=10)
plot(asca.selected)
print(sort(colnames[asca.fit$Model.bab$leverage>asca.fitted$lev_limit]))
print(sort(masigpro.fit$summary[,2]))
venn_list <- list(maSigpro = sort(masigpro.fit$summary[,2]), ASCA.gene = sort(colnames[asca.fit$Model.bab$leverage>asca.fitted$lev_limit]))
venn::venn(venn_list)
```
