...
masigpro.fit1 <- maSigPro(df.final.masigpro,masigpro.design,degree=degree,rsq = 0.8)
knitr::kable(masigpro.fit1$summary)
toplot <- rownames(masigpro.fit1$sig.genes$treatment2vstreatment1$sig.profiles)
```
```{r}
masigpro_coe <- masigpro.fit1$sig.genes$treatment2vstreatment1$coefficients
kable(glimpse(head(masigpro_coe)))
colnames(masigpro_coe) <- c('beta0','beta1','delta0','delta1','gamma0','gamma1')
```
...
asca.fit.split <- ASCA.2f_leverage(X = t(df.final.asca),Designa = mx$j, Designb = mx$i,type = 2, Fac = Fac)
