...
There are 12 metabolites with the differential expression shown in the above linear function and 100 metabolites with flat expression.
...
trend.toplot <- data.frame(replicate=rep(1:(i*j),each=r),time=rep(time,each=r),treatment=factor(rep(1:i,each=j*r)),metabolite=df.final[2,])
