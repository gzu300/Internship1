...
ftrs_in_pat <- list(15,15,1,55-31)
...
ggplot(trend.toplot,aes(x=time,y=metabolite,color=treatment))+
  ...
  labs(title = 'pattern')
sim.asca.pattern <- plot_a_metabolite(df.final,mean,'one',tag='A')
```
...
leverages_spe <- plot.leverage_spe(asca.fitted,0.2,size = 10,tag='B')
lev.spe.sim.asca <- leverages_spe
plot(leverages_spe)
```
