...
##example of differentially expressed variable is shown above.
...
plot.leverage_spe_original(df.final,asca.fit,groups,R=20)
```
...
```{r PCA ld unormed,eval=F, include=T}
PCA.GENES.unorm_loading<-function(X)
{
  ...
  eigen<-eigen(Xoff%*%t(Xoff)/(p-1))#calculates eigen of c*cT, eigen() returns unit vector
  var<-cbind(eigen$values/sum(eigen$values),cumsum(eigen$values/sum(eigen$values)))#variability explained and accumulated variability explained
