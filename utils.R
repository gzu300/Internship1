...
wrap.permutation.info <- function(df.final,asca.fit,groups,R,which_leverage,alpha=0.05,...){
  ...
  Nulldist_plot <- plot.NullDistribution(permu.data,asca.fit$Model.bab$leverage,groups,lev.lim,R,alpha)
...
plot.NullDistribution <- function(permu.data,model.data,colnames,cutoff,R,alpha){
